...
class delaunay:
    ...
    def drawPoints(self, p):
        #randColor = (random.randrange(0, 255, 1), random.randrange(0, 255, 1), random.randrange(0, 255, 1))
        WHITE = (255, 255, 255)
        pygame.draw.circle(self.sc, WHITE, (int(p.x), int(p.y)), 1)

    def drawEdge(self, e):
        WHITE = (255, 255, 255)
        pygame.draw.line(self.sc, WHITE, (e.point1.x, e.point1.y), (e.point2.x, e.point2.y), 1)

    def chekDoubly(self, e):
        for i in range(0, len(e)):
            for j in range(i+1, len(e)):
                if ((e[i].point1 == e[j].point1) & (e[i].point2 == e[j].point2)) | ((e[i].point1 == e[j].point2) & (e[i].point2 == e[j].point1)):
                    e[i].isBad = 1
                    e[j].isBad = 1
                    break
